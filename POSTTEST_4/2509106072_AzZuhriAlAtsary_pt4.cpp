...
NodeTiket *topStack = nullptr;

void push(NodeTiket *data) {
    NodeTiket *nodeBaru = new NodeTiket;
    nodeBaru->namaPenumpang = data->namaPenumpang;
    nodeBaru->asal          = data->asal;
    nodeBaru->tujuan        = data->tujuan;
    nodeBaru->namaKereta    = data->namaKereta;
...
void pop() {
    ...
    cout << "Hapus riwayat: " << topStack->namaPenumpang << endl;
    NodeTiket *temp = topStack;
...
void peekStack() {
    ...
    cout << "Terakhir (Stack): " << topStack->namaPenumpang
        << " (" << topStack->namaKereta << ")\n";
}
...
NodeTiket *frontQueue = nullptr;
NodeTiket *rearQueue  = nullptr;

void enqueue(NodeTiket *data) {
    NodeTiket *nodeBaru = new NodeTiket;
    nodeBaru->namaPenumpang = data->namaPenumpang;
    nodeBaru->asal          = data->asal;
    nodeBaru->tujuan        = data->tujuan;
    ...
    cout << "Masuk antrian: " << nodeBaru->namaPenumpang << endl;
}

bool dequeue(NodeTiket *hasil) {
    if (frontQueue == nullptr) {
        cout << "Queue underflow: Antrian kosong.\n";
        return false;
    }
    hasil->namaPenumpang = frontQueue->namaPenumpang;
    hasil->asal          = frontQueue->asal;
    hasil->tujuan        = frontQueue->tujuan;
    hasil->namaKereta    = frontQueue->namaKereta;

    NodeTiket *temp = frontQueue;
...
void peekQueue() {
    ...
    cout << "Depan (Queue): " << frontQueue->namaPenumpang << endl;
}
...
int main() {
    ...
    do {
        ...
        if (pilih == 1) {
            ...
            NodeTiket data;
            cin.ignore();
            cout << "Nama Penumpang: "; getline(cin, data.namaPenumpang);
            cout << "Asal: ";           getline(cin, data.asal);
            cout << "Tujuan: ";         getline(cin, data.tujuan);
            cout << "Kereta: ";         getline(cin, data.namaKereta);
            data.next = nullptr;
            enqueue(&data);
            system("pause");
            ...
            hasil.next = nullptr;
            if (dequeue(&hasil)) {
                cout << "Diproses: " << hasil.namaPenumpang << endl;
                push(&hasil);
            }
            system("pause");
